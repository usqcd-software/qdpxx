...
function build_chroma { 
	...
	then
	   ...
	   dslash_config=${7}/bin/wfm-config
	   dslash_cxxflags=`${dslash_config} --cxxflags`
	   dslash_ldflags=`${dslash_config} --ldflags`
	   dslash_libs=`${dslash_config} --libs`
	else
	   pab_dslash="";
	   dslash_cxxflags=""
	   dslash_ldflags=""
	...
	pushd ${builddir}
	export PATH=/home/ed/bj/bin:$PATH
	command="${chromadir}/configure CXXFLAGS=\"${gmpincdir} ${dslash_cxxflags}\" LDFLAGS=\"${gmplibdir} ${dslash_ldflags}\" "
	command=${command}" LIBS=\"${gmplib} ${dslash_libs}\" "
