...
public:
  ...
  // this buffer is used for the fat surface buffer when shifting along x direction
  mutable void * shadow_buf;

  template<typename T> const T* getShadowBufPtr() const { return static_cast<T*>(shadow_buf); }
#endif
  int srcnum, dstnum;
  QMP_msgmem_t msg[2];
...
class FnMapRsrcMatrix {
  ...
  std::pair< int , std::vector<FnMapRsrc*> >* get(int _destNode,int _srcNode,
    ...
    if ( pos.second.size() ==  pos.first ) {
      // QDP_info("allocate and setup new rsrc-obj (destnode= %d, sndmsgsize=%d) \n", _destNode, _sendMsgSize);
...
public:
  ~RsrcWrapper() {
    if (rAlloc) {
      pPair->first--;
      // QDP_info("wrapper destructor: %p counting down to %d\n", pPair, pPair->first);
#if QDP_DEBUG >= 3
      if (pPair->first < 0)
	QDP_error_exit("<0");
#endif
  ...
  const FnMapRsrc& getResource(int srcnum_, int dstnum_) {
    ...
    // QDP_info("wrapper %p: returning obj %d \n",pPair,  pPair->first);
