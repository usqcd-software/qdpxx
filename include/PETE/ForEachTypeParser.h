...
#ifndef POOMA_PETE_FOREACHTYPEPARSER_H
#define POOMA_PETE_FOREACHTYPEPARSER_H

//-----------------------------------------------------------------------------
// Overview: 
//
//   ForEachTypeParser is like ForEach except that it traverses the parse
...
//   in essence, when the ForEachTypeParser::apply moves down and back up
...
//
//   OpVisitor is a new class that visits the "value" node prior,
...
// Full Description:
//
// The ForEachTypeParser struct implements a template meta-program
...
// ForEach. 
//
// The ForEachTypeParser struct defines:
//
//   typename ForEachTypeParser<Expr,FTag,VTag,CTag>::Type_t
//
...
//
//     OpVisitor<...>::start(e.value_m,v);
//
//     left_val = ForEachTypeParser<...>::apply(e.left_m,f,v,c),
//
//     OpVisitor<...>::visit(e.value_m,v);
//
//     right_val = ForEachTypeParser<...>::apply(e.right_m,f,v,c),
//
//     retval = TagCombineTypeParsere<...>::
//               apply(left_val, right_val, e.value_m, c)
//
//     OpVisitor<...>::finish(e.value_m,v);
//
//     return retval;
//
...
// leaf nodes. 
//
// The OpVisitor is specialized to perform specific actions both at
...
// OpVisitor::apply must be of the type Op. This usually means
// that OpVisitor::apply will just return e.value_m after it does
// its calculation.
//
...
struct OpVisitor 
{
  static void start(const Op & op, const VTag & v) {}
  static void center(const Op & op, const VTag & v) { }
  static bool visit(const Op & op, const VTag & v) {}
  static void finish(const Op & op, const VTag & v) { }
};  


// 
// struct ForEachTypeParser
//
...
struct ForEachTypeParser<Reference<T>,FTag,VTag,CTag>
{
...
template<class Op, class A, class FTag, class VTag, 
  class CTag>
struct ForEachTypeParser<UnaryNode<Op, A>, FTag, VTag, CTag>
{
  typedef ForEachTypeParser<A, FTag, VTag, CTag> ForEachA_t;
  typedef OpVisitor<Op, VTag>          Visitor_t;
  ...
  static Type_t apply(const UnaryNode<Op, A> &expr, const FTag &f, 
		      const VTag &v, const CTag &c)
  {
    if (!Visitor_t::visit(expr.operation(),v))
      return false;
...
/*!
 * struct ForEachTypeParser for BinaryNode
 */

template<class Op, class A, class B, class FTag, class VTag, 
  class CTag>
struct ForEachTypeParser<BinaryNode<Op, A, B>, FTag, VTag, CTag>
{
  typedef ForEachTypeParser<A, FTag, VTag, CTag> ForEachA_t;
  typedef ForEachTypeParser<B, FTag, VTag, CTag> ForEachB_t;
  typedef OpVisitor<Op, VTag>                Visitor_t;
...
template<class Op, class A, class B, class C, class FTag, class VTag, 
  class CTag>
struct ForEachTypeParser<TrinaryNode<Op, A, B, C>, FTag, VTag, CTag>
{
  typedef ForEachTypeParser<A, FTag, VTag, CTag> ForEachA_t;
  typedef ForEachTypeParser<B, FTag, VTag, CTag> ForEachB_t;
  typedef ForEachTypeParser<C, FTag, VTag, CTag> ForEachC_t;
  typedef OpVisitor<Op, VTag>                Visitor_t;
...
struct Combine2<A, B, Op, NullTagTP>
{
  typedef int Type_t;
  static Type_t combine(A, B, Op, NullTagTP)
    { return 0; }
};
...
// ACL:rcsinfo
// ----------------------------------------------------------------------
// $RCSfile: ForEachTypeParser.h,v $   $Author: edwards $
