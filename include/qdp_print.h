// -*- C++ -*-
// $Id: qdp_print.h,v 1.1 2004-07-15 04:01:04 edwards Exp $

/*! @file
 * @brief Print out PETE expressions
 *
 * Diagnostics to print an expression tree. Note, if new operators are
 * added, there should be corresponding definitions for TagVisitor.
 */

#ifndef QDP_PRINT_INCLUDE
#define QDP_PRINT_INCLUDE
...
//inline
void printExprTree(StandardOutputStream& os, 
		   const QDPType<T,C>& dest, const Op& op, const QDPExpr<RHS,C1>& rhs)
{
  ...
  LeafFunctor<OLattice<T>,PrintTag>::apply(static_cast<const C&>(dest),PrintTag(os));
  os << " ";
...
struct PrintTag
{
  mutable StandardOutputStream &os_m;
  PrintTag(StandardOutputStream &os) : os_m(os) {}
};
...
struct LeafFunctor<QDPType<T,C>, PrintTag>
{
  ...
  static int apply(const QDPType<T,C> &s, const PrintTag &f)
    { 
      const C& me = static_cast<const C&>(s);
      return LeafFunctor<C,PrintTag>::apply(me, f);
    }
};
