...
namespace QDP 
{
  ...
  {
  public:
    index_t operator() (const multi1d<index_t>& coordinate) const {return 0;}
    index_t numSubsets() const {return 1;}
  };
  ...
  {
  public:
    index_t operator() (const multi1d<index_t>& coordinate) const
      {
	index_t sum = 0;
	for(auto m=0; m < coordinate.size(); ++m)
	  sum += coordinate[m];

	return sum & 1;
      }

    index_t numSubsets() const {return 2;}
  };
  ...
  public:
    index_t operator() (const multi1d<index_t>& coordinate) const
      {
	index_t initial_color = 0;
	for(auto m=Nd-1; m >= 0; --m)
	  initial_color = (initial_color << 1) + (coordinate[m] & 1);

	int cb = 0;
	for(index_t m=0; m < Nd; ++m)
	  cb += coordinate[m] >> 1;
    ...
    index_t numSubsets() const {return 1 << (Nd+1);}
  };
  ...
  void Subset::make(bool _rep, index_t _start, index_t _end, multi1d<index_t>* ind, int cb, Set* _set)
  {
    ordRep    = _rep;
    startSite = _start;
    endSite   = _end;
