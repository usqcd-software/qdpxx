...
namespace QDP {
  ...
  void FnMapRsrc::setup(int _destNode,int _srcNode,int _sendMsgSize,int _rcvMsgSize) {
    ...
    int srcnum=_rcvMsgSize;
    int dstnum=_sendMsgSize;
    ...
    shadow_buf_mem = QMP_allocate_aligned_memory(srcnum,QDP_ALIGNMENT_SIZE, (QMP_MEM_COMMS|QMP_MEM_FAST)); // packed receive data
    if(shadow_buf_mem == 0x0 ) { 
      shadow_buf_mem = QMP_allocate_aligned_memory(srcnum, QDP_ALIGNMENT_SIZE, QMP_MEM_COMMS); 
      if( shadow_buf_mem == 0x0 ) { 
	QDP_error_exit("Unable to allocate recv_buf_mem\n");
      }
    }
...
#if defined(ARCH_PARSCALARVEC)
    shadow_buf=QMP_get_memory_pointer(shadow_buf_mem);
#endif    
...
#if defined(ARCH_PARSCALARVEC)
      QMP_free_memory(shadow_buf_mem);
#endif      
    }
  }
