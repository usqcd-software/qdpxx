
/*! @file
 * @brief Parscalar layout routines
 * 
 * Layout routines for parscalar implementation
 * QDP data parallel interface
 *
 * Layout
 *
 * This routine provides various layouts, including
 *    lexicographic
 *    2-checkerboard  (even/odd-checkerboarding of sites)
 *    32-style checkerboard (even/odd-checkerboarding of hypercubes)
 */

#include "qdp_diagnostics.h"

#include "qdp.h"
#include "qdp_util.h"
#include "qdp_allocator.h"
#include "qmp.h"


namespace QDP 
{

  //----------------------------------Only defined here
  template<class T>
  std::ostream& operator<<(std::ostream& s, const multi1d<T>& s1)
  {
    for(int i=0; i < s1.size(); ++i)
      s << " " << s1[i];

    return s;
  }

  // Only relevant in case of pool allocator.
  float pool_size_in_gb = 8.0;


  //-----------------------------------------------------------------------------
  namespace Layout
  {

    //-----------------------------------------------------
    //! Local data specific to all architectures
    /*! 
     * NOTE: the disadvantage to using a struct to keep things together is
     * that subsequent groupings of namespaces can not just add onto the
     * current namespace.
     */
    struct LocalLayout_t
    {
      //! Total lattice volume
      index_t vol;

      //! Lattice size
      multi1d<index_t> nrow;

      //! Subgrid lattice volume
      index_t subgrid_vol;

      //! Subgrid lattice size
      multi1d<index_t> subgrid_nrow;

      //! Logical node coordinates
      multi1d<index_t> logical_coord;

      //! Logical system size
      multi1d<index_t> logical_size;

      //! Node rank
      index_t node_rank;

      //! Total number of nodes
      index_t num_nodes;

      bool iogrid_defined;
      index_t  num_iogrid;
      multi1d<index_t> iogrid;

    } _layout;


    //-----------------------------------------------------
    // Functions

    //! Main destruction routine
    void destroy() {RNG::finalizeRNG();}

    //! Set virtual grid (problem grid) lattice size
    void setLattSize(const multi1d<int>& nrows) {
	_layout.nrow.resize(nrows.size());
        for(auto i=0; i < nrows.size(); ++i) _layout.nrow[i] = static_cast<index_t>(nrows[i]);
    }

    //! Set SMP flag -- true if using smp/multiprocessor mode on a node
    /*! For now, this is ignored */
    void setSMPFlag(bool flag) {}

    //! Set number of processors in a multi-threaded implementation
    /*! For now, this is ignored */
    void setNumProc(index_t N) {}

    //! Virtual grid (problem grid) lattice size
    const multi1d<index_t>& lattSize() {return _layout.nrow;}

    //! Total lattice volume
    index_t vol() {return _layout.vol;}

    //! Subgrid lattice volume
    index_t sitesOnNode() {return _layout.subgrid_vol;}
//! Returns whether this is the primary node
    bool primaryNode() {return (_layout.node_rank == 0) ? true : false;}

    //! Subgrid (grid on each node) lattice size
    const multi1d<index_t>& subgridLattSize() {return _layout.subgrid_nrow;}

    //! Returns the node number of this node
    index_t nodeNumber() {return _layout.node_rank;}

    //! Returns the number of nodes
    index_t numNodes() {return _layout.num_nodes;}

    //! Returns the logical node coordinates for this node
    const multi1d<index_t>& nodeCoord() {return _layout.logical_coord;}

    //! Returns the logical size of this machine
    const multi1d<index_t>& logicalSize() {return _layout.logical_size;}

    //! Returns the node number given some logical node coordinate
    /*! This is not meant to be speedy */
    index_t getNodeNumberFrom(const multi1d<index_t>& node_coord) 
    {
      multi1d<int> crds(Nd);
      for(auto i=0; i < Nd; ++i) { crds[i] = static_cast<int>(node_coord[i]); }

      return static_cast<index_t>(QMP_get_node_number_from(crds.slice()));
    }

    //! Returns the logical node coordinates given some node number
    /*! This is not meant to be speedy */
    multi1d<index_t> getLogicalCoordFrom(index_t node) 
    {
      multi1d<index_t> node_coord(Nd);
      const int* node_crd = QMP_get_logical_coordinates_from(node);  // QMP mallocs here

      for(int i=0; i < Nd; ++i)
	node_coord[i] = static_cast<index_t>(node_crd[i]);

      // Hackery as free cannot take a const void*, so grab
      // node_crd with a non const pointer
      int* non_const_node_crd = const_cast<int*>(node_crd);
      free(non_const_node_crd);   // free up QMP's memory
      return node_coord;
    }

    //! Return the smallest lattice size per node allowed
    multi1d<index_t> minimalLayoutMapping();

    //! Initializer for layout
    void init()
    {
      _layout.num_nodes =  static_cast<index_t>(QMP_get_number_of_nodes());

      _layout.node_rank = static_cast<index_t>(QMP_get_node_number());
		
      // Default should be current behaviour (as of 7/4/12)
      // No iogrid is defined -> each node is its own I/O node
      _layout.iogrid_defined = false;  
      _layout.num_iogrid = _layout.num_nodes;
    }



    //! The linearized site index for the corresponding lexicographic site
    index_t linearSiteIndex(index_t site)
    { 
      auto coord = crtesn(site, lattSize());
    
      return linearSiteIndex(coord);
    }


	  
    //! check if I/O grid is defined
    /*! Always defined for scalar node: it is 1x1x1x1 */
    bool isIOGridDefined(void) QDP_CONST 
    { 
      return _layout.iogrid_defined; 
    }
	  
    //! number of I/O nodes
    index_t numIONodeGrid(void) QDP_CONST
    {
		
		  
      return _layout.num_iogrid;
		  
    }
	  
    //! Set the I/O Node grid -- satisfy interface
    void setIONodeGrid(const multi1d<index_t>& io_grid) 
    {
      _layout.iogrid.resize(Nd);	
      for(auto mu=0; mu < Nd; mu++) {
	_layout.iogrid[mu] = io_grid[mu];
      }
      _layout.num_iogrid = io_grid[0];
      for(auto mu=1; mu < Nd; mu++) { 
	_layout.num_iogrid *= io_grid[mu];
      }
      _layout.iogrid_defined = true;
    }
	  
    //! Get the I/O Node grid
    const multi1d<index_t>& getIONodeGrid() QDP_CONST
    {
      return _layout.iogrid;
    }
	  
    //! Initializer for all the layout defaults
    void initDefaults()
    {
#if QDP_DEBUG >= 2
      QDP_info("Create default subsets");
#endif
      // Default set and subsets
      initDefaultSets();

      // Default maps
      initDefaultMaps();

      // Initialize RNG
      RNG::initDefaultRNG();

      // Set default profile level
      setProfileLevel(getProgramProfileLevel());
    }


    //! Main lattice creation routine
    void create()
    {
      if ( ! QDP_isInitialized() )
	QDP_error_exit("QDP is not initialized");

      if (_layout.nrow.size() != Nd)
	QDP_error_exit("dimension of lattice size not the same as the default");

      _layout.vol=1;
      for(auto i=0; i < Nd; ++i) 
	_layout.vol *= _layout.nrow[i];
  
#if QDP_DEBUG >= 2
      QDP_info("vol=%d",_layout.vol);
#endif

#if QDP_DEBUG >= 2
      QDP_info("Initialize layout");
#endif

      // Simple check - we insist here that the total volume is divisible
      // by the number of processors. Will also insist that the problem
      // size is regular on each node
      if (_layout.vol % numNodes() != 0)
	QDP_error_exit("Layout::create - problem size not divisible by number of processors");

    
      // Return the smallest lattice size per node allowed
      multi1d<index_t> min_dim = minimalLayoutMapping();

      // Another sanity check - if the specified lattice size is not a multiple
      // of the minimal size, then barf
      for(auto i=0; i < Nd; ++i)
	if (_layout.nrow[i] % min_dim[i] != 0)
	  QDP_error_exit("Layout::create - Error: problem size not multiple of smallest size allowed for this type of layout");


      // Now, layout the machine. Note, if the logical_machine size was set previously
      // it will be used
      multi1d<index_t> nrow(Nd);
      for(int i=0; i < Nd; ++i)
	nrow[i] = _layout.nrow[i] / min_dim[i];

      int nrow_c[Nd];
      for(auto i=0; i < Nd; ++i)nrow_c[i] =static_cast<int>(nrow[i]);
      QMP_layout_grid(nrow_c,Nd);


      // Pull out useful stuff
      const int* phys_size = QMP_get_logical_dimensions();
      const int* phys_coord = QMP_get_logical_coordinates();

      _layout.subgrid_nrow.resize(Nd);
      _layout.logical_coord.resize(Nd);
      _layout.logical_size.resize(Nd);

      _layout.subgrid_vol = 1;

      for(int i=0; i < Nd; ++i)
      {
	_layout.logical_coord[i] = static_cast<index_t>(phys_coord[i]);
	_layout.logical_size[i] = static_cast<index_t>(phys_size[i]);
	_layout.subgrid_nrow[i] = _layout.nrow[i] / _layout.logical_size[i];

	_layout.subgrid_vol *= _layout.subgrid_nrow[i];
      }

      // Diagnostics
      QDPIO::cout << "Lattice initialized:\n";
      QDPIO::cout << "  problem size =";
      for(int i=0; i < Nd; ++i)
	QDPIO::cout << " " << _layout.nrow[i];
      QDPIO::cout << std::endl;

      QDPIO::cout << "  layout size =";
      for(int i=0; i < Nd; ++i)
	QDPIO::cout << " " << nrow[i];
      QDPIO::cout << std::endl;

      QDPIO::cout << "  logical machine size =";
      for(int i=0; i < Nd; ++i)
	QDPIO::cout << " " << _layout.logical_size[i];
      QDPIO::cout << std::endl;

      QDPIO::cout << "  subgrid size =";
      for(int i=0; i < Nd; ++i)
	QDPIO::cout << " " << _layout.subgrid_nrow[i];
      QDPIO::cout << std::endl;

      QDPIO::cout << "  total number of nodes = " << Layout::numNodes() << std::endl;
      QDPIO::cout << "  total volume = " << _layout.vol << std::endl;
      QDPIO::cout << "  subgrid volume = " << _layout.subgrid_vol << std::endl;
      if ( _layout.iogrid_defined ) { 
        QDPIO::cout << "  Number of IO nodes = " << _layout.num_iogrid << std::endl;
        QDPIO::cout << "  IO grid size =";
        for(int i=0; i < Nd; i++) 
 	  QDPIO::cout << " " << _layout.iogrid[i];
        QDPIO::cout << std::endl;
      } 

      // Sanity check - check the QMP node number functions
      for(int node=0; node < Layout::numNodes(); ++node)
      { 
	multi1d<index_t> coord = Layout::getLogicalCoordFrom(node);
	index_t node2 = Layout::getNodeNumberFrom(coord);

	if (node != node2)
	  QDP_error_exit("Layout::create - Layout problems, the QMP logical to physical node map functions do not work correctly with this lattice size");
      }

      // Sanity check - check the layout functions make sense
#if QDP_DEBUG >= 2
      // BJ: Put this into a debug loop as it can take a serious amount of time for a really
      // large lattice
      for(auto site=0; site < vol(); ++site) 
      {
	auto coord1 = crtesn(site, lattSize());

	auto linear = linearSiteIndex(coord1);
	auto node   = nodeNumber(coord1);

	auto coord2 = siteCoords(node,linear);
	auto j = local_site(coord2, lattSize());
      
#if QDP_DEBUG >= 3
	QDP_info("site= %d   coord= %d %d %d %d   linear= %d node=%d   crd=%d %d %d %d   j= %d",
		 site,coord1[0],coord1[1],coord1[2],coord1[3],
		 linear,node,
		 coord2[0],coord2[1],coord2[2],coord2[3],
		 j);
#endif
	if (site != j)
	  QDP_error_exit("Layout::create - Layout problems, the layout functions do not work correctly with this lattice size");
      }
#endif


      size_t pool_size_in_MB = static_cast<size_t>(floor(pool_size_in_gb*1024.0));

      Allocator::theQDPAllocator::Instance().init(pool_size_in_MB);

      // Initialize various defaults
      initDefaults();


      QDPIO::cout << "Finished lattice layout" << std::endl;

#if QDP_DEBUG >= 2
      QDP_info("Layout successfully initialized");
#endif
    }
  }


  //-----------------------------------------------------------------------------
#if QDP_USE_LEXICO_LAYOUT == 1

QDPXX_MESSAGE("Using a lexicographic layout")

  namespace Layout
  {
    //! The linearized site index for the corresponding coordinate
    /*! This layout is a simple lexicographic lattice ordering */
    index_t linearSiteIndex(const multi1d<index_t>& coord)
    {
      multi1d<index_t> tmp_coord(Nd);

      for(auto i=0; i < coord.size(); ++i)
	tmp_coord[i] = coord[i] % Layout::subgridLattSize()[i];
    
      return local_site(tmp_coord, Layout::subgridLattSize());
    }


    //! The node number for the corresponding lattice coordinate
    /*! This layout is a simple lexicographic lattice ordering */
    index_t nodeNumber(const multi1d<index_t>& coord)
    {
      multi1d<index_t> tmp_coord(Nd);

      for(auto i=0; i < coord.size(); ++i)
	tmp_coord[i] = coord[i] / Layout::subgridLattSize()[i];
    
      return Layout::getNodeNumberFrom(tmp_coord);
    }


    //! Returns the lattice site for some input node and linear index
    /*! This layout is a simple lexicographic lattice ordering */
    multi1d<index_t> siteCoords(index_t node, index_t linear)
    {
      multi1d<index_t> coord = getLogicalCoordFrom(node);

      // Get the base (origins) of the absolute lattice coord
      coord *= Layout::subgridLattSize();
    
      // Find the coordinate within a node and accumulate
      // This is a lexicographic ordering
      coord += crtesn(linear, Layout::subgridLattSize());

      return coord;
    }


    //! Return the smallest lattice size per node allowed
    /*! This layout is a simple lexicographic lattice ordering */
    multi1d<index_t> minimalLayoutMapping()
    {
      multi1d<idex_t> dim(Nd);
      dim = 1;

      return dim;
    }
  }

  //-----------------------------------------------------------------------------

#elif QDP_USE_CB2_LAYOUT == 1

QDPXX_MESSAGE("Using a 2 checkerboard (red/black) layout")

  namespace Layout
  {
    //! The linearized site index for the corresponding coordinate
    /*! This layout is appropriate for a 2 checkerboard (red/black) lattice */
    index_t linearSiteIndex(const multi1d<index_t>& coord)
    {
      auto subgrid_vol_cb = Layout::sitesOnNode() >> 1;
      multi1d<index_t> subgrid_cb_nrow = Layout::subgridLattSize();
      subgrid_cb_nrow[0] >>= 1;

      index_t cb = 0;
      for(auto m=0; m < Nd; ++m)
	cb += coord[m];
      cb &= 1;

      multi1d<index_t> subgrid_cb_coord(Nd);
      subgrid_cb_coord[0] = (coord[0] >> 1) % subgrid_cb_nrow[0];
      for(auto i=1; i < Nd; ++i)
	subgrid_cb_coord[i] = coord[i] % subgrid_cb_nrow[i];
    
      return local_site(subgrid_cb_coord, subgrid_cb_nrow) + cb*subgrid_vol_cb;
    }


    //! The node number for the corresponding lattice coordinate
    /*! 
     * This layout is appropriate for a 2 checkerboard (red/black) lattice,
     * but to find the nodeNumber this function resembles a simple lexicographic 
     * layout
     */
    index_t nodeNumber(const multi1d<index_t>& coord)
    {
      multi1d<index_t> tmp_coord(Nd);

      for(auto i=0; i < coord.size(); ++i)
	tmp_coord[i] = coord[i] / Layout::subgridLattSize()[i];
    
      return Layout::getNodeNumberFrom(tmp_coord);
    }


    //! Reconstruct the lattice coordinate from the node and site number
    /*! 
     * This is the inverse of the nodeNumber and linearSiteIndex functions.
     * The API requires this function to be here.
     */
    multi1d<index_t> siteCoords(index_t node, index_t linearsite) // ignore node
    {
      auto subgrid_vol_cb = Layout::sitesOnNode() >> 1;
      multi1d<index_t> subgrid_cb_nrow = Layout::subgridLattSize();
      subgrid_cb_nrow[0] >>= 1;

      // Get the base (origins) of the absolute lattice coord
      multi1d<index_t> coord = getLogicalCoordFrom(node);
      coord *= Layout::subgridLattSize();
    
      index_t cb = linearsite / subgrid_vol_cb;
      multi1d<index_t> tmp_coord = crtesn(linearsite % subgrid_vol_cb, subgrid_cb_nrow);

      // Add on position within the node
      // NOTE: the cb for the x-coord is not yet determined
      coord[0] += 2*tmp_coord[0];
      for(auto m=1; m < Nd; ++m)
	coord[m] += tmp_coord[m];

      // Determine cb including global node cb
      int cbb = cb;
      for(auto m=1; m < Nd; ++m)
	cbb += coord[m];
      coord[0] += (cbb & 1);

      return coord;
    }


    //! Return the smallest lattice size per node allowed
    /*! This layout is appropriate for a 2 checkerboard (red/black) lattice */
    multi1d<index_t> minimalLayoutMapping()
    {
      multi1d<index_t> dim(Nd);
      dim = 1;
      dim[0] = 2;       // must have multiple length 2 for cb

      return dim;
    }
  }

  //-----------------------------------------------------------------------------

#elif QDP_USE_CB3D_LAYOUT == 1

QDPXX_MESSAGE("Using a 3D checkerboard (red/black) layout")

  namespace Layout
  {
    //! The linearized site index for the corresponding coordinate
    /*! This layout is appropriate for a 2 checkerboard (red/black) lattice */
    index_t linearSiteIndex(const multi1d<index_t>& coord)
    {
      index_t subgrid_vol_cb = Layout::sitesOnNode() / 2;
      multi1d<index_t> subgrid_cb_nrow = Layout::subgridLattSize();
      subgrid_cb_nrow[0] /= 2;

      index_t cb = 0;
      for(auto  m=0; m < Nd-1; ++m) { // Nd-1 checkerboard
	cb += coord[m];
      }
      cb &= 1;

      multi1d<index_t> subgrid_cb_coord(Nd);
      subgrid_cb_coord[0] = (coord[0] / 2) % subgrid_cb_nrow[0];
      for(auto i=1; i < Nd; ++i)
	subgrid_cb_coord[i] = coord[i] % subgrid_cb_nrow[i];
    
      return local_site(subgrid_cb_coord, subgrid_cb_nrow) + cb*subgrid_vol_cb;
    }


    //! The node number for the corresponding lattice coordinate
    /*! 
     * This layout is appropriate for a 2 checkerboard (red/black) lattice,
     * but to find the nodeNumber this function resembles a simple lexicographic 
     * layout
     */
    index_t nodeNumber(const multi1d<index_t>& coord)
    {
      multi1d<index_t> tmp_coord(Nd);

      for(auto i=0; i < coord.size(); ++i)
	tmp_coord[i] = coord[i] / Layout::subgridLattSize()[i];
    
      return Layout::getNodeNumberFrom(tmp_coord);
    }


    //! Reconstruct the lattice coordinate from the node and site number
    /*! 
     * This is the inverse of the nodeNumber and linearSiteIndex functions.
     * The API requires this function to be here.
     */
    multi1d<index_t> siteCoords(index_t node, index_t linearsite) // ignore node
    {
      index_t subgrid_vol_cb = Layout::sitesOnNode() / 2;
      multi1d<index_t> subgrid_cb_nrow = Layout::subgridLattSize();
      subgrid_cb_nrow[0] /= 2;


      // Get the base (origins) of the absolute lattice coord
      multi1d<index_t> coord = getLogicalCoordFrom(node);
      coord *= Layout::subgridLattSize();
    
      index_t cb = linearsite / subgrid_vol_cb;
      multi1d<index_t> tmp_coord = crtesn(linearsite % subgrid_vol_cb, subgrid_cb_nrow);

      

      // Add on position within the node
      // NOTE: the cb for the x-coord is not yet determined
      coord[0] += 2*tmp_coord[0];
      for(auto m=1; m < Nd; ++m)
	coord[m] += tmp_coord[m];



      // Determine cb including global node cb
      int cbb = cb;
      for(auto m=1; m < Nd-1; ++m)
	cbb += coord[m];

   
      coord[0] += (cbb & 1);

      return coord;
    }


    //! Return the smallest lattice size per node allowed
    /*! This layout is appropriate for a 2 checkerboard (red/black) lattice */
    multi1d<index_t> minimalLayoutMapping()
    {
      multi1d<index_t> dim(Nd);
      multi1d<index_t> lsize=Layout::lattSize(); // Full lattice size;
      dim = 1;
      dim[0] = 2;       // must have multiple length 2 for cb

      return dim;
    }
  }


  //-----------------------------------------------------------------------------

#elif QDP_USE_CB32_LAYOUT == 1

QDPXX_MESSAGE("Using a 32 checkerboard layout")

#error "THIS BIT STILL UNDER CONSTRUCTION"

  namespace Layout
  {
    //! The linearized site index for the corresponding coordinate
    /*! This layout is appropriate for a 32-style checkerboard lattice */
    index_t linearSiteIndex(const multi1d<index_t>& coord)
    {
      index_t subgrid_vol_cb = Layout::sitesOnNode() >> (Nd+1);
      multi1d<index_t> subgrid_cb_nrow = Layout::subgridLattSize();
      subgrid_cb_nrow[0] >>= 2;
      for(auto i=1; i < Nd; ++i) 
	subgrid_cb_nrow[i] >>= 1;

      int subl = coord[Nd-1] & 1;
      for(auto m=Nd-2; m >= 0; --m)
	subl = (subl << 1) + (coord[m] & 1);

      int cb = 0;
      for(int m=0; m < Nd; ++m)
	cb += coord[m] >> 1;

      subl += (cb & 1) << Nd;   // Final color or checkerboard

      // Construct the checkerboard lattice coord
      multi1d<index_t> subgrid_cb_coord(Nd);

      subgrid_cb_coord[0] = (coord[0] >> 2) % subgrid_cb_nrow[0];
      for(auto i=1; i < Nd; ++i)
	subgrid_cb_coord[i] = (coord[i] >> 1) % subgrid_cb_nrow[i];
    
      return local_site(subgrid_cb_coord, subgrid_cb_nrow) + subl*subgrid_vol_cb;
    }


    //! The node number for the corresponding lattice coordinate
    /*! 
     * This layout is appropriate for a 32 checkerboard (red/black) lattice,
     * but to find the nodeNumber this function resembles a simple lexicographic 
     * layout
     */
    index_t nodeNumber(const multi1d<index_t>& coord)
    {
      multi1d<index_t> tmp_coord(Nd);

      for(auto i=0; i < coord.size(); ++i)
	tmp_coord[i] = coord[i] / Layout::subgridLattSize()[i];
    
      return Layout::getNodeNumberFrom(tmp_coord);
    }


    //! Reconstruct the lattice coordinate from the node and site number
    /*! 
     * This is the inverse of the nodeNumber and linearSiteIndex functions.
     * The API requires this function to be here.
     */
    multi1d<index_t> siteCoords(index_t node, index_t linearsite) // ignore node
    {
      index_t subgrid_vol_cb = Layout::sitesOnNode() >> (Nd+1);
      multi1d<index_t> subgrid_cb_nrow = Layout::subgridLattSize();
      subgrid_cb_nrow[0] >>= 2;
      for(auto i=1; i < Nd; ++i) 
	subgrid_cb_nrow[i] >>= 1;

      // Get the base (origins) of the absolute lattice coord
      multi1d<index_t> coord = Layout::getLogicalCoordFrom(node);
      coord *= Layout::subgridLattSize();
    
      index_t subl = linearsite / subgrid_vol_cb;
      multi1d<index_t> tmp_coord = crtesn(linearsite % subgrid_vol_cb, subgrid_cb_nrow);

      // Add on position within the node
      // NOTE: the cb for the x-coord is not yet determined
      coord[0] += tmp_coord[0] << 2;
      for(auto m=1; m < Nd; ++m)
	coord[m] += tmp_coord[m] << 1;

      int cb = 0;
      for(auto m=1; m<Nd; ++m)
	cb += coord[m];
      cb &= 1;

      subl ^= (cb << Nd);
      for(auto m=0; m<Nd; ++m)
	coord[m] ^= (subl & (1 << m)) >> m;
      coord[0] ^= (subl & (1 << Nd)) >> (Nd-1);   // this gets the hypercube cb

      return coord;
    }


    //! Return the smallest lattice size per node allowed
    /*! This layout is appropriate for a 32-style checkerboard lattice */
    multi1d<index_t> minimalLayoutMapping()
    {
      multi1d<index_t> dim(Nd);

      dim[0] = 4;       // must have multiple length 2 for cb and hypercube
      for(auto m=1; m < Nd; ++m)
	dim[m] = 2;     //  must have multiple length 2 for hypercube

      return dim;
    }
  }

#else

#error "no appropriate layout defined"

#endif

  //-----------------------------------------------------------------------------


} // namespace QDP;
