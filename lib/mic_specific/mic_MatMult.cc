...
#include "qdp.h"
#include "mic_specific/mic_MatMult.h"
#include "immintrin.h"

namespace QDP
{

int debug_mic_MatMult=0;
...
{
if (debug_mic_MatMult) QDPIO::cout << "specialized QDP_M=M*M" << endl;
			//w=u*v;
...
// 					vrow2[k] = _mm512_i32logather_pd(indexv, (void*)&v.elem(k,0).imag(), scale);
	...
	for(int i=0; i < 3; ++i)
	{
		...
		for (int k=0; k<3; ++k)
		{
			...
			wvec = _mm512_fnmadd_pd(uvec,vrow1[k],wvec);
			//wvec = _mm512_mask3_fnmadd_pd(uvec,vrow1[k],wvec,0x3F);
...
//					__declspec(align(64)) double uu[8]={u.elem(i,k).imag(), u.elem(i,k).real(), u.elem(i,k).imag(), u.elem(i,k).real(), u.elem(i,k).imag(), u.elem(i,k).real(),0,0};
			...
			wvec = _mm512_fnmadd_pd(uvec,vrow2[k],wvec);
			//wvec = _mm512_mask3_fnmadd_pd(uvec,vrow2[k],wvec,0x3F);
