...
//! Initializer for generic map constructor
  void Map::make(const MapFunc& func)
  {
    ...
    goffsetsId = QDPCache::Instance().registrateOwnHostMem( sizeof(int)*goffsets.size() , (void*)goffsets.slice() );
    ...
    roffsetsId = QDPCache::Instance().registrateOwnHostMem( sizeof(int)*roffsets.size() , (void*)roffsets.slice() );
    soffsetsId = QDPCache::Instance().registrateOwnHostMem( sizeof(int)*soffsets.size() , (void*)soffsets.slice() );
#endif
  ...
  void Map::make(const MapFunc& func)
  {
    ...
    if (goffsets.size()==0)
      QDP_error_exit("gsoffsets empty");
    if (ind_array.size()==0)
      QDP_error_exit("ind_arra empty");

    roffsetsId = QDPCache::Instance().registrateOwnHostMem( sizeof(int)*roffsets.size() , (void*)roffsets.slice() );
    ind_arrayId = QDPCache::Instance().registrateOwnHostMem( sizeof(int)*ind_array.size() , (void*)ind_array.slice() );
    soffsetsId = QDPCache::Instance().registrateOwnHostMem( sizeof(int)*soffsets.size() , (void*)soffsets.slice() );
