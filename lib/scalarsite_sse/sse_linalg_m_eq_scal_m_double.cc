...
#include <xmmintrin.h>
  ...
  void ssed_m_eq_scal_m(REAL64* m2, REAL64* a, REAL64 *m1, int n_mat)
  {
    ...
    for(int i=0; i < n_mat; i++) { 
      ...
      _mm_store_pd(m2_p, tmp2);
      
      tmp3= _mm_load_pd(m1_p+2);
      tmp4= _mm_mul_pd(scalar, tmp3);
      _mm_store_pd(m2_p+2, tmp4);

      tmp5= _mm_load_pd(m1_p+4);
      tmp6= _mm_mul_pd(scalar, tmp5);
      _mm_store_pd(m2_p+4, tmp6);

      tmp7= _mm_load_pd(m1_p+6);
      tmp8= _mm_mul_pd(scalar, tmp7);
      _mm_store_pd(m2_p+6, tmp8);

      tmp9= _mm_load_pd(m1_p+8);
      tmp10= _mm_mul_pd(scalar, tmp9);
      _mm_store_pd(m2_p+8, tmp10);

      tmp11= _mm_load_pd(m1_p+10);
      tmp12= _mm_mul_pd(scalar, tmp11);
      _mm_store_pd(m2_p+10, tmp12);

      tmp13= _mm_load_pd(m1_p+12);
      tmp14= _mm_mul_pd(scalar, tmp13);
      _mm_store_pd(m2_p+12, tmp14);

      tmp15= _mm_load_pd(m1_p+14);
      tmp4= _mm_mul_pd(scalar, tmp15);
      _mm_store_pd(m2_p+14, tmp4);

      tmp5= _mm_load_pd(m1_p+16);
      tmp6= _mm_mul_pd(scalar, tmp5);
  ...
  void ssed_m_muleq_scal(REAL64* m, REAL64* a, int n_mat)
  {
    ...
    for(int i=0; i < n_mat; i++) { 
      tmp1= _mm_load_pd(m1_p);
      tmp2= _mm_mul_pd(scalar, tmp1);
      _mm_store_pd(m1_p, tmp2);
      
      tmp3= _mm_load_pd(m1_p+2);
      tmp4= _mm_mul_pd(scalar, tmp3);
      _mm_store_pd(m1_p+2, tmp4);

      tmp5= _mm_load_pd(m1_p+4);
      tmp6= _mm_mul_pd(scalar, tmp5);
      _mm_store_pd(m1_p+4, tmp6);

      tmp7= _mm_load_pd(m1_p+6);
      tmp8= _mm_mul_pd(scalar, tmp7);
      _mm_store_pd(m1_p+6, tmp8);

      tmp9= _mm_load_pd(m1_p+8);
      tmp10= _mm_mul_pd(scalar, tmp9);
      _mm_store_pd(m1_p+8, tmp10);

      tmp11= _mm_load_pd(m1_p+10);
      tmp12= _mm_mul_pd(scalar, tmp11);
      _mm_store_pd(m1_p+10, tmp12);

      tmp13= _mm_load_pd(m1_p+12);
      tmp14= _mm_mul_pd(scalar, tmp13);
      _mm_store_pd(m1_p+12, tmp14);

      tmp15= _mm_load_pd(m1_p+14);
      tmp4= _mm_mul_pd(scalar, tmp15);
      _mm_store_pd(m1_p+14, tmp4);

      tmp5= _mm_load_pd(m1_p+16);
      tmp6= _mm_mul_pd(scalar, tmp5);
      _mm_store_pd(m1_p+16, tmp6);
