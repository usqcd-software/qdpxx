// $Id: sse_blas_vaxmyz4_double.cc,v 1.3 2008-06-23 14:19:43 bjoo Exp $
...
void vaxmyz4(REAL64 *Out,REAL64 *scalep,REAL64 *InScale, REAL64 *Add,int n_4vec)
{
 __m128d scalar;
  ...
  if( dlen > L2SizeInKB ) { 
    ...
    _mm_prefetch((char *)(add_p+24),_MM_HINT_NTA);
    _mm_prefetch((char *)(in_p+24),_MM_HINT_NTA);
    ...
    for(int i=0; i < n_4vec-1; i++) { 
      _mm_prefetch((char *)(add_p+72),_MM_HINT_NTA);
      _mm_prefetch((char *)(in_p+72),_MM_HINT_NTA);
      ...
      _mm_stream_pd(out_p-4, out2);
      add2 = _mm_load_pd(add_p+2);
      in2  = _mm_load_pd(in_p+2);
      tmp2 = _mm_mul_pd(scalar, in2);
      out2 = _mm_sub_pd(tmp2,add2);
      
      _mm_stream_pd(out_p-2, out3);
      add3 = _mm_load_pd(add_p+4);
      in3 = _mm_load_pd(in_p+4);
      tmp3 = _mm_mul_pd(scalar, in3);
      out3 = _mm_sub_pd(tmp3,add3);
